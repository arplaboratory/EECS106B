...
def imag_loss(
  ...
  # SkyDreamer smoothness regularizer (paper §III.C). Penalises the squared
  # difference of the actor mean across consecutive imagined steps; applied
  # only to the mean (not the full distribution) to avoid clashing with the
  # entropy term. Coefficient 0.002 in SkyDreamer.
  if smooth and smooth > 0.0:
    smooth_term = 0.0
    for k, v in policy.items():
      mu = getattr(v, 'mean', None)
      if mu is None:
        continue
      diff = mu[:, 1:] - mu[:, :-1]
      smooth_term = smooth_term + jnp.mean(jnp.square(diff), axis=-1)
    if not isinstance(smooth_term, float):
      policy_loss = policy_loss + smooth * smooth_term[:, :-1]
      metrics['smooth'] = smooth_term.mean()
